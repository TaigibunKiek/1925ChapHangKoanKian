(function () {
  // TODO edit
  const baseUrl = '/1925ChapHangKoanKian';
  const dataFolderPath = '/chuliau';
  const dataNamePrefix = '/1925ChapHangKoanKian_Page_';
  const dataNameNumberCount = 3;
  const dataNameSuffix = '.jpg';
  const lastPageNumber = 170;
  ...
  for (i = imgSourcesMin; i < imgSourcesMax + 1; i++) {
    ...
    imgSources.push({
      type: 'image',
      url: `..${baseUrl}${dataFolderPath}${dataNamePrefix}${numberText}${dataNameSuffix}`
    })
  }
  const viewer = OpenSeadragon({
    id: 'viewer-block',
    prefixUrl: `..${baseUrl}/assets/images/openseadragon/`,
